...
Features:
  - Auto-installs/updates required packages.
...
def simulate_leveraged_cumulative_return(df, leverage=5):
    ...
    (This function uses .to_numpy() to avoid alignment issues.)
    """
    df['daily_return'] = df['price'].pct_change().fillna(0)
    df['strategy_return'] = leverage * df['daily_return'].to_numpy() * df['signal'].to_numpy()
